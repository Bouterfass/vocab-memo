...
export const TestScreen: React.FC<TestScreenProps> = ({ onBack }) => {
  ...
  const handleValidate = (e: React.FormEvent) => {
    ...
    const targetAnswer =
      direction === 'EN_FR' ? currentWord.french : currentWord.english;

    const normalizedUser = normalizeString(userAnswer);
    const normalizedTargets = targetAnswer
      .split('/')
      .map(ans => normalizeString(ans))
      .filter(Boolean);

    // 1) Match exact (après normalisation)
    const isExactMatch = normalizedTargets.includes(normalizedUser);

    // 2) Match "typo tolérée" via Levenshtein
    const isCloseMatch =
      !isExactMatch &&
      normalizedTargets.some(target => {
        const dist = levenshteinDistance(normalizedUser, target);

        // tolérance dynamique :
        // - mots très courts : 0 erreur
        // - mots moyens : 1 erreur
        // - mots longs : 2 erreurs
        const len = target.length;
        let maxAllowed = 0;

        if (len <= 4) maxAllowed = 1;
        else if (len <= 7) maxAllowed = 2;
        ...
        <div className="flex-1 flex flex-col justify-center text-center space-y-6">
          ...
          <div className="space-y-4">
            <button
              ...
              className={`w-full p-4 rounded-xl shadow-neu-out text-sm font-semibold transition-transform active:scale-95 outline-none focus:ring-2 focus:ring-neu-accent/20 ${direction === 'EN_FR' ? 'ring-2 ring-neu-accent text-neu-accent' : 'text-neu-text'
                }`}
            >
              Anglais ➔ Français
            </button>
            <button
              ...
              className={`w-full p-4 rounded-xl shadow-neu-out text-sm font-semibold transition-transform active:scale-95 outline-none focus:ring-2 focus:ring-neu-accent/20 ${direction === 'FR_EN' ? 'ring-2 ring-neu-accent text-neu-accent' : 'text-neu-text'
                }`}
            >
              Français ➔ Anglais
            </button>
  ...
  return (
    <div className="p-6 pt-8 min-h-[500px] flex flex-col">
